...
async function loadHistory() {
    try {
        ...
        const { data } = await api.get(`/booking/history/${user_id}`)
            ...
            container.innerHTML += `
                <div class="history-card" style="animation-delay:${index * 0.15}s">
                    ...
                    <div class="history-info">
                        ...
                        ${books.status === "borrowed" ? `<p id="time-${books.id}" class="countdown"></p>` : ""}
        ...
        console.error("โหลดประวัติไม่ได้", err)
    }
}
...
function startCountdown(endDate, bookingId) {
    ...
    function updateCountdown() {
        ...
        element.innerText = `เวลาที่เหลือ: ${days} วัน ${hours} ชม ${minutes} นาที`
    }
...
async function returnBook(book_id) {
    ...
    try {
        ...
        const { data } = await api.post("/booking/return", { book_id })
        ...
        Swal.fire('เกิดข้อผิดพลาด', err.response?.data?.message || 'คืนหนังสือไม่ได้', 'error')
